const routineCardData: any[] = [
  {
    routineId: '0',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'RunAndLead',
    text: 'asdnioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '1',
    imgSrc:
      'https://contents.creators.mypetlife.co.kr/content/uploads/2020/05/01085845/c320a089abe34b72942aeecc9b568295',
    title: '아무거나챌린지몇자까지가능?',
    text: '아니 그러니까 나는 열심히 만든다고 만들었는데 디자인이 맘에 안든데 근데 어떻게 개선해야될지를 가르쳐주질 않어 그럼 내가 어떻게 해야해 정말 너무 어렵다 디자인',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '2',
    imgSrc: './icons/heart.png',
    title: 'test2',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  dasefasefasefasefasefasefasefasefasefasefasefasefasefasioandddfasdfasasdfasdfasdfdfasdfasdfasdfsaseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '3',
    imgSrc: './icons/rightArrow.png',
    title: 'test3',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '4',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test4',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '5',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test5',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '6',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test6',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '7',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test7',
    text: 'asdnfioasneioansefmasioefmioasdfasdfasdfasdfasdfasdfasdfasdfasdfasdfasdfsenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '8',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test8',
    text: 'asdnfioasneioansefmasioefmioasenasdfasdfasdfasdfasdfasdfasdffioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '9',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test9',
    text: 'asdnioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '10',
    imgSrc:
      'https://contents.creators.mypetlife.co.kr/content/uploads/2020/05/01085845/c320a089abe34b72942aeecc9b568295',
    title: 'test10',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '11',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test11',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  dasefasefasefasefasefasefasefasefasefasefasefasefasefasioandddfasdfasasdfasdfasdfdfasdfasdfasdfsaseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '12',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test12',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '13',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test13',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '14',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test14',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '15',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test15',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '16',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test16',
    text: 'asdnfioasneioansefmasioefmioasdfasdfasdfasdfasdfasdfasdfasdfasdfasdfasdfsenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '17',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test17',
    text: 'asdnfioasneioansefmasioefmioasenasdfasdfasdfasdfasdfasdfasdffioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '18',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test18',
    text: 'asdnioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '19',
    imgSrc:
      'https://contents.creators.mypetlife.co.kr/content/uploads/2020/05/01085845/c320a089abe34b72942aeecc9b568295',
    title: 'test19',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '20',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test20',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  dasefasefasefasefasefasefasefasefasefasefasefasefasefasioandddfasdfasasdfasdfasdfdfasdfasdfasdfsaseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '21',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test21',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '22',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test22',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '23',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test23',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '24',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test24',
    text: 'asdnfioasneioansefmasioefmioasenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '25',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test25',
    text: 'asdnfioasneioansefmasioefmioasdfasdfasdfasdfasdfasdfasdfasdfasdfasdfasdfsenfioasneoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
  {
    routineId: '26',
    imgSrc:
      'https://cdn.crowdpic.net/list-thumb/thumb_l_9FA2E3C7E87309B6B344204FCDBFF3CE.jpg',
    title: 'test26',
    text: 'asdnfioasneioansefmasioefmioasenasdfasdfasdfasdfasdfasdfasdffioasnasdddddddddddddddddeoifnasoef  ioanseoifnaiosenfi',
    timeText: '11.12 ~ 11.25',
    location: 'gaepo',
  },
];

export default function getData() {
  return routineCardData;
}
